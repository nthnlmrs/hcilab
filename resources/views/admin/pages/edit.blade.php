@section('section_name', 'Edit Page')
<x-app-layout>
    <div class="pt-6 pb-20 px-4 max-w-2xl mx-auto">
        <div class="flex items-center mb-6">
            ...
            <h1 class="font-serif text-xl font-bold text-museum-green">Edit Page</h1>
        </div>
        ...
        <form action="{{ route('admin.pages.update', $page) }}" method="POST" id="page-form" enctype="multipart/form-data">
            ...
            <div class="space-y-4 mb-6">
                <div class="bg-white rounded-2xl p-4 shadow-sm">
                    ...
                    <input type="text" name="title" id="title" value="{{ old('title', $page->title) }}" required class="w-full rounded-xl border-gray-300 focus:border-museum-green focus:ring-museum-green" placeholder="e.g., Arca Dwarapala">
                </div>

                <div class="bg-white rounded-2xl p-4 shadow-sm">
                    <label for="description" class="block text-xs font-bold text-gray-500 uppercase mb-1">Short Description</label>
                    <textarea name="description" id="description" rows="2" class="w-full rounded-xl border-gray-300 focus:border-museum-green focus:ring-museum-green" placeholder="Brief summary of the artifact...">{{ old('description', $page->description) }}</textarea>
                </div>
                ...
                <div class="bg-white rounded-2xl p-4 shadow-sm">
                    ...
                    @if($page->cover_image)
                        <div class="mt-2 text-[10px] text-gray-500 flex items-center gap-2">
                            <img src="{{ asset('storage/' . $page->cover_image) }}" class="w-8 h-8 rounded object-cover">
    ...
    <script>
        ...
        const existingBlocks = @json($page->blocks);
        ...
        function addBlock(type, data = null, content = null) {
            let contentHtml = '';
            let title = '';
            let icon = '';
            ...
            if (type === 'title') {
                ...
                contentHtml = `
                    <div class="grid grid-cols-2 gap-2 mb-2">
                        <select name="blocks[${blockIndex}][data][level]" class="text-xs rounded-lg border-gray-300">
                            <option value="h1" ${bData.level === 'h1' ? 'selected' : ''}>Heading 1</option>
                            <option value="h2" ${(!bData.level || bData.level === 'h2') ? 'selected' : ''}>Heading 2</option>
                            <option value="h3" ${bData.level === 'h3' ? 'selected' : ''}>Heading 3</option>
                        </select>
                        <select name="blocks[${blockIndex}][data][align]" class="text-xs rounded-lg border-gray-300">
                            <option value="left" ${bData.align === 'left' ? 'selected' : ''}>Left</option>
                            <option value="center" ${(!bData.align || bData.align === 'center') ? 'selected' : ''}>Center</option>
                            <option value="right" ${bData.align === 'right' ? 'selected' : ''}>Right</option>
                        </select>
                    </div>
                    <input type="text" name="blocks[${blockIndex}][content]" value="${escapeHtml(bContent)}" placeholder="Enter title..." required class="w-full text-lg font-bold rounded-xl border-gray-300 focus:border-museum-green focus:ring-museum-green">
                `;
                ...
                contentHtml = `
                    <select name="blocks[${blockIndex}][data][align]" class="text-xs rounded-lg border-gray-300 mb-2">
                        <option value="left" ${(!bData.align || bData.align === 'left') ? 'selected' : ''}>Left</option>
                        <option value="center" ${bData.align === 'center' ? 'selected' : ''}>Center</option>
                        <option value="justify" ${bData.align === 'justify' ? 'selected' : ''}>Justify</option>
                    </select>
                    <textarea name="blocks[${blockIndex}][content]" placeholder="Enter description content..." rows="4" required class="w-full rounded-xl border-gray-300 focus:border-museum-green focus:ring-museum-green">${escapeHtml(bContent)}</textarea>
                `;
            } else if (type === 'image') {
                title = 'Image Block';
                icon = 'fa-image';
                ...
                if (bData.url) {
                    existingImg = `
                        <input type="hidden" name="blocks[${blockIndex}][existing_image]" value="${escapeHtml(bData.url)}">
                        <div class="mt-2 text-[10px] text-gray-500 flex items-center gap-2 mb-2">
                            <img src="/storage/${escapeHtml(bData.url)}" class="w-8 h-8 rounded object-cover">
                            <span>Current Image</span>
                        </div>
                    `;
                }
                ...
                contentHtml = `
                    <input type="file" name="blocks[${blockIndex}][image_file]" ${bData.url ? '' : 'required'} class="w-full text-xs text-gray-500 mb-2">
                    ${existingImg}
                    <input type="text" name="blocks[${blockIndex}][data][alt]" value="${escapeHtml(bData.alt)}" placeholder="Alt text (for accessibility)" class="w-full text-xs rounded-lg border-gray-300 mb-1">
                    <input type="text" name="blocks[${blockIndex}][data][caption]" value="${escapeHtml(bData.caption)}" placeholder="Optional caption" class="w-full text-xs rounded-lg border-gray-300">
                `;
            } else if (type === 'card') {
                title = 'Card Block';
                icon = 'fa-id-card';
                ...
                if (bData.image) {
                    existingImg = `
                        <input type="hidden" name="blocks[${blockIndex}][existing_image]" value="${escapeHtml(bData.image)}">
                        <div class="mt-2 text-[10px] text-gray-500 flex items-center gap-2 mb-2">
                            <img src="/storage/${escapeHtml(bData.image)}" class="w-8 h-8 rounded object-cover">
                            <span>Current Image</span>
                        </div>
                    `;
                }
                ...
                contentHtml = `
                    <div class="space-y-2">
                        ...
                        ${existingImg}
                        <input type="text" name="blocks[${blockIndex}][data][title]" value="${escapeHtml(bData.title)}" placeholder="Card Title" class="w-full text-sm font-bold rounded-lg border-gray-300">
                        <textarea name="blocks[${blockIndex}][data][desc]" placeholder="Card Description" rows="2" class="w-full text-sm rounded-lg border-gray-300">${escapeHtml(bData.desc)}</textarea>
                        <div class="flex gap-2">
                             <input type="text" name="blocks[${blockIndex}][data][btn_text]" value="${escapeHtml(bData.btn_text)}" placeholder="Button Text" class="w-1/2 text-xs rounded-lg border-gray-300">
                             <input type="text" name="blocks[${blockIndex}][data][btn_link]" value="${escapeHtml(bData.btn_link)}" placeholder="Button Link" class="w-1/2 text-xs rounded-lg border-gray-300">
                        </div>
                    </div>
                `;
            } else if (type === 'button') {
                ...
                contentHtml = `
                    <div class="grid grid-cols-2 gap-2">
                        <input type="text" name="blocks[${blockIndex}][data][text]" value="${escapeHtml(bData.text)}" placeholder="Button Label" required class="w-full rounded-lg border-gray-300">
                        <input type="text" name="blocks[${blockIndex}][data][url]" value="${escapeHtml(bData.url)}" placeholder="URL (e.g., https://...)" required class="w-full rounded-lg border-gray-300">
                    </div>
                    <div class="mt-2">
                        <label class="inline-flex items-center text-xs text-gray-500">
                            <input type="checkbox" name="blocks[${blockIndex}][data][new_tab]" value="1" ${bData.new_tab ? 'checked' : ''} class="rounded text-museum-green mr-2"> Open in new tab
                        </label>
                    </div>
                `;
            }
        ...
        if (existingBlocks && existingBlocks.length > 0) {
            existingBlocks.forEach(block => {
                addBlock(block.type, block.data, block.content);
            });
        }
    </script>
</x-app-layout>
